...
探索要素はなし。**どのレベル番号が安全か**を覚えながら、SANITY を保って現実への帰還を目指す。

公開: https://kaikomziu.github.io/backrooms/

## 収録内容
- **Backrooms Wiki 日本語版**（backrooms.fandom.com/ja）の実レベルを収録
- 無印通常階層 33 + 通常階層 η（エータ層）30 の計 63 レベル + ゴール「現実」
- ルートは各レベルページの「出口」「入口」節から採取した一方通行グラフ
- 危険度は Wiki の「危険度 X/5」を SANITY 減少量に変換：0〜2→0（安全）/ 3→−8 / 4→−24 / 5→−45
- **Level 0 → 0η** が η 層への入口。ゴールは **Level 3999 →「現実」**

## 遊び方
- 画面には現在のレベル（番号＋Wiki名）と、行ける先のレベル番号ボタンだけが出る
- ボタンを押すと即移動。安全なら無傷、危険なレベルほど SANITY が大きく減る
- **危険度はサイト上に一切表示されない**。SANITY の増減だけが手がかり。どの番号が危ないかは自分で覚える
- 「到達記録」には行ったレベル（番号＋名前）と回数だけが残る（死んでも残る）
- SANITY 0 で振り出し
- 「現実」に着けば脱出クリア。行き止まりに入るとそのランは終了

レベルとルートは下の `level` コマンドで自由に組み替えられる。
